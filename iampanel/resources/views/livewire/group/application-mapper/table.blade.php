...
<div>
    <div class="flex flex-col lg:flex-row lg:justify-between lg:items-center mb-4">
        ...
        <div class="mt-4 lg:mt-0">
            <label>
                <a href="{{ route('groups.assign-role', $group_id) }}" class="mt-1 inline-block lg:inline w-full lg:w-48 py-2 px-4 rounded-md font-semibold text-white bg-blue-500 focus-control text-center">Tambah Akses Peran</a>
            </label>
        </div>
    </div>
                    ...
                    @foreach ($role_mappings as $key => $role_mapping)
                        <tr class="text-gray-700 dark:text-gray-400">
                            ...
                            <td class="px-4 py-3 text-sm">
                                ...
                                <button title="Hapus akses peran aplikasi" wire:key="role-{{ $role_mapping['role_id'] }}" wire:click="deleteAssignedClientRoleConfirm('{{ $role_mapping['client_id'] }}', '{{ json_encode($role_mapping) }}')" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-red-600 rounded-lg dark:text-gray-400 focus-red-control">
